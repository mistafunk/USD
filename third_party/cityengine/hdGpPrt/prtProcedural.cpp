...
public:
  PrtProcedural(const SdfPath &proceduralPrimPath)
   ...
   TF_STATUS("PrtProcedural ctor");
  }

  DependencyMap
  UpdateDependencies(const HdSceneIndexBaseRefPtr &inputScene) override {
    TF_STATUS("PrtProcedural::UpdateDependencies");
  ...
  ChildPrimTypeMap Update(
   ...
   TF_STATUS("PrtProcedural::Update");
    ...
    // TODO: check if any work is needed at all

    HdSceneIndexPrim sourceMeshPrim = inputScene->GetPrim(args.sourceMeshPath);
    if (sourceMeshPrim.primType == HdPrimTypeTokens->mesh) {
      // TODO: get the mesh ... sourceMeshPrim.dataSource.
    }

    // TODO: line up data source mesh for prt::generate...
...
public:
  PrtProceduralPlugin() { TF_STATUS("loading PrtProcedurlaPlugin"); }
...
TF_REGISTRY_FUNCTION(TfType) {
  TF_STATUS("loading PrtProceduralPlugin");
