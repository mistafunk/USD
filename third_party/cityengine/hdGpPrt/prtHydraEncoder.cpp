...
#include "prtCallbacks.h" // we assume USD compiler is compatible with PRT compiler
...
constexpr const wchar_t* ENC_NAME = L"Hydra Encoder";
constexpr const wchar_t* ENC_DESCRIPTION = L"Encodes generated geometry into Hydra data sources.";
...
const prtx::EncodePreparator::PreparationFlags PREP_FLAGS =
        prtx::EncodePreparator::PreparationFlags()
                ...
                .indexSharing(prtx::EncodePreparator::PreparationFlags::
...
HydraEncoder::HydraEncoder(const std::wstring& id, const prt::AttributeMap* options,
                           prt::Callbacks* callbacks)
    : prtx::GeometryEncoder(id, options, callbacks) {}

void HydraEncoder::init(prtx::GenerateContext&) {
	prt::Callbacks* cb = getCallbacks();
	auto* oh = dynamic_cast<PrtCallbacks*>(cb);
	if (oh == nullptr)
		throw prtx::StatusException(prt::STATUS_ILLEGAL_CALLBACK_OBJECT);
}

void HydraEncoder::encode(prtx::GenerateContext& context, size_t initialShapeIndex) {
	const prtx::InitialShape& initialShape = *context.getInitialShape(initialShapeIndex);
	auto* cb = static_cast<PrtCallbacks*>(getCallbacks());
	...
	prtx::ReportingStrategyPtr reportsCollector{prtx::LeafShapeReportingStrategy::create(
	        context, initialShapeIndex, reportsAccumulator)};
...
void HydraEncoder::convertGeometry(const prtx::InitialShape& initialShape,
                                   const prtx::EncodePreparator::InstanceVector& instances,
                                   PrtCallbacks* cb, prt::Cache* cache) {
	...
	cb->addMesh(initialShape.getName(), sg.mCoords.data(), sg.mCoords.size(), sg.mNormals.data(),
	            sg.mNormals.size(), sg.mCounts.data(), sg.mCounts.size(), sg.mVertexIndices.data(),
	            sg.mVertexIndices.size(), sg.mNormalIndices.data(), sg.mNormalIndices.size(),

	            puvs.first.data(), puvs.second.data(), puvCounts.first.data(),
	            puvCounts.second.data(), puvIndices.first.data(), puvIndices.second.data(),
	            sg.mUvs.size(),
...
void HydraEncoder::finish(prtx::GenerateContext& /*context*/) {}

HydraEncoderFactory* HydraEncoderFactory::createInstance() {
	prtx::EncoderInfoBuilder encoderInfoBuilder;

	encoderInfoBuilder.setID(ENC_ID_HYDRA);
	...
	return new HydraEncoderFactory(encoderInfoBuilder.create());
}
